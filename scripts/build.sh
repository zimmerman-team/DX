...
if [ "$MODE" = "dev" ]; then
  sudo rm .env
  sudo ln .env.dev .env
  sudo docker compose -f docker-compose.dev.yml up --build $I1 $I2 $I3 $I4 $I5
elif [ "$MODE" = "prod" ]; then
  sudo rm .env
  sudo ln .env.prod .env
  sudo docker compose up -d --build $I1 $I2 $I3 $I4 $I5
elif [ "$MODE" = "staging" ]; then
  sudo rm .env
  sudo ln .env.staging .env
  sudo docker compose -f docker-compose.staging.yml up -d --build $I1 $I2 $I3 $I4 $I5
elif [ "$MODE" = "test" ]; then
  sudo rm .env
  sudo ln .env.test .env
  sudo docker compose -f docker-compose.test.yml up -d --build $I1 $I2 $I3 $I4 $I5
else
  echo "Invalid mode. Use 'dev', 'test', 'staging' or 'prod'."
  exit 1
fi
